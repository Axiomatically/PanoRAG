...
You cannot change storage implementation selection after adding documents to PanoRAG. Data migration from one storage implementation to another is not supported yet. For further information, please read the sample env file or config.ini file.

### PanoRAG API Server Command Line Options
...
| --key                 | None          | API key for authentication. Protects the PanoRAG server against unauthorized access                                            |
