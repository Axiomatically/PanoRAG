...
import os
import logging
from panorag.kg.shared_storage import finalize_share_data
from panorag.utils import setup_logger
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to panorag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to panorag.log
...
def on_starting(server):
    ...
    # Log the location of the PanoRAG log file
    print(f"PanoRAG log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("panorag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up panorag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("panorag."):
