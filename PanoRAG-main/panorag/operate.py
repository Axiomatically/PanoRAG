...
async def extract_keywords_only(
    ...
    It ONLY extracts keywords (hl_keywords, ll_keywords), and expands them using topic tree.
    """
    ...
    # 4. Prompt for keyword extraction
    kw_prompt = PROMPTS["keywords_extraction"].format(
        query=text, examples=examples, language=language, history=history_context
    )
    logger.debug(f"[kg_query]Prompt Tokens: {len(encode_string_by_tiktoken(kw_prompt))}")
    ...
    # 7. Keyword Expansion using topic tree
    expanded = await expand_keywords_with_vdb(
    query=text,
    high_level_keywords=hl_keywords,
    low_level_keywords=ll_keywords,
    vector_db_path=global_config.get("vdb_topic_path", "./mix/vdb_topic_tree.json"),
    embedding_func=global_config["embedding_func"],
    embedding_dim=global_config.get("embedding_dim", 1536),
    top_k_children=3,
    max_keywords=10,
    similarity_threshold=0.30
)

    hl_keywords_exp = expanded.get("high_level_keywords", [])
    ll_keywords_exp = expanded.get("low_level_keywords", [])
    
    print("[Keywords] After EXPAND")
    print("  High-level:", hl_keywords_exp)
    print("  Low-level :", ll_keywords_exp)

    return hl_keywords_exp, ll_keywords_exp
...
async def _get_2hop_entities(
    initial_entities: list[str],
    knowledge_graph_inst: BaseGraphStorage,
    max_hop: int = 2,
) -> tuple[list[str], dict[int, set[str]]]:
    visited = set(initial_entities)
    frontier = set(initial_entities)
    hop_map = {0: set(initial_entities)}  # hop index → entity set

    for hop in range(1, max_hop + 1):
        next_frontier = set()
        all_edges = await asyncio.gather(
            *[knowledge_graph_inst.get_node_edges(ent) for ent in frontier]
        )
        for edges in all_edges:
            if edges:
                next_frontier.update([tgt for _, tgt in edges])
        next_frontier -= visited
        if not next_frontier:
            break
        hop_map[hop] = next_frontier.copy()
        visited |= next_frontier
        frontier = next_frontier

    return list(visited), hop_map
...
async def _get_node_data(
    ...
    matched_entities = [r["entity_name"] for r in results]
    hop = 2
    all_entity_names, hop_map = await _get_2hop_entities(matched_entities, knowledge_graph_inst, max_hop=hop)

    node_datas = await asyncio.gather(
        *[knowledge_graph_inst.get_node(name) for name in all_entity_names]
    )
    node_degrees = await asyncio.gather(
        *[knowledge_graph_inst.node_degree(name) for name in all_entity_names]
    )

    node_datas = [
        {**n, "entity_name": name, "rank": d}
        for name, n, d in zip(all_entity_names, node_datas, node_degrees)
        if n is not None
    ]
