...
import numpy as np
import tiktoken
from panorag.prompt import PROMPTS
from dotenv import load_dotenv
from nano_vectordb import NanoVectorDB

# Use TYPE_CHECKING to avoid circular imports
if TYPE_CHECKING:
    from panorag.base import BaseKVStorage

# use the .env that is inside the current folder
# allows to use different .env file for each panorag instance
...
# Initialize logger
logger = logging.getLogger("panorag")
...
class LightragPathFilter(logging.Filter):
    """Filter for panorag logger to filter out frequent path access logs"""
...
def setup_logger(
    ...
    Args:
        ...
        log_file_path: Path to the log file. If None and file logging is enabled, defaults to panorag.log in LOG_DIR or cwd
    ...
    if enable_file_logging:
        ...
        if log_file_path is None:
            ...
            log_file_path = os.path.abspath(os.path.join(log_dir, "panorag.log"))
...
async def aexport_data(
    ...
    if file_format == "csv":
        ...
        with open(output_path, "w", encoding="utf-8") as mdfile:
            mdfile.write("# PanoRAG Data Export\n\n")
...
def check_storage_env_vars(storage_name: str) -> None:
    ...
    """
    from panorag.kg import STORAGE_ENV_REQUIREMENTS
...
async def expand_keywords_with_vdb(
    query: str,
    ...
    from .utils_topictree import TopicTreeVectorDB
    import numpy as np
    engine = TopicTreeVectorDB(vector_db_path, embedding_func, embedding_dim)

    hl_result = set(high_level_keywords)
    ll_result = set(low_level_keywords)

    hl_query = ", ".join(high_level_keywords)
    hl_embedding = (await embedding_func([hl_query]))[0]  

    hl_matches = engine.vdb.query(
        query=hl_embedding,
        top_k=3,
        better_than_threshold=similarity_threshold,
    )
    hl_expanded = []
    for node in hl_matches:
        parents = engine.expand_up(node)
        parent_topics = [p["topic"] for p in parents if p]
        hl_result.update([t for t in parent_topics if t.lower() not in {"root", "topic"}])
        #hl_result.update(parent_topics)
        hl_expanded.extend(parent_topics)

    ll_query = ", ".join(low_level_keywords)
    ll_embedding = (await embedding_func([ll_query]))[0] 

    ll_matches = engine.vdb.query(
        query=ll_embedding,
        top_k=3,
        better_than_threshold=similarity_threshold,
    )
    children = []
    for node in ll_matches:
        expanded = engine.expand_down(node)
        children.extend(expanded)
    ll_expanded = []
    if children:
        embeddings = await embedding_func([query] + texts)
        query_vec = np.array(embeddings[0])
        child_vecs = np.array(embeddings[1:])
        scores = np.dot(child_vecs, query_vec)
        scored = sorted(zip(texts, scores), key=lambda x: x[1], reverse=True)
        for kw, score in scored:
            filtered = [
            kw for kw, score in scored
            if score >= similarity_threshold
        ][:top_k_children]
        ll_result.update(filtered)
        ll_expanded = filtered
    final_hl = high_level_keywords + [kw for kw in hl_result if kw not in high_level_keywords]
    final_ll = low_level_keywords + [kw for kw in ll_result if kw not in low_level_keywords]
    return {
        "high_level_keywords": final_hl[:max_keywords],
        "low_level_keywords": final_ll[:max_keywords],
    }
